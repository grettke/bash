...
Primary prompt is the one that you see all the time.
...
Secondary prompt is the one that you see when you break a command into
multiple lines.
...
Tertiary prompt is the one that you see when you are presented with a menu
...
Quaternary prompt is what you see when you enable debugging with ~set -x~.
