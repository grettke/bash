...
Brew Ruby. Note that the installer mentions you should add the Gem bin dir to the path. That directory is different from the Ruby version. Being unsure how to automate this it is going to be manual for now.
