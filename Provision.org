...
- Use SI standard base-10 for:
  - network bandwidth (for example, 6 Mbit/s or 50 kB/s) disk sizes (for
    example, 500 GB hard drive or 4.7 GB DVD)
  - Suffixes
...
function u2a {
  uni2ascii \
    ⟪u2a-format⟫ \
    ⟪u2a-unenclose-circles-and-parens⟫ \
    ⟪u2a-strip-diacritics⟫ \
    ⟪u2a-convert-char-to-approximate-ascii-equivelant⟫ \
    ⟪u2a-convert-stylistic-variants⟫ \
    ⟪u2a-expand-to-multichar⟫ \
    ⟪u2a-arguments⟫
}
#+end_src

- "Generate hexadecimal UTF-8 with each byte's hex preceded by an =-sign (e.g.
...
#+name: u2a-convert-stylistic-variants
#+begin_src sh
-f
#+end_src
