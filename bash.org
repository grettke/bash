...
digraph version5 {
  ...
  subgraph cluster_1 {
    ...
    BashGlobalNonInteractive[shape = "note" label = ".bash_global_non_interactive\n- «BashCore»\n- Unset interactive functions"]
    BashGlobalInteractive[shape = "note" label = ".bash_global_interactive\n- «BashCore»\n- «BashInteractiveCore»"]
  }
  subgraph cluster_2 {
    label = "Interactive [Non-]Login Shell\n\n(unique user\nconfiguration files)";
    BashProfile[shape = "note" label = ".bash_profile\n- Session start commands, once"]
    BashRc[shape = "note" label = ".bashrc\n- source #BASH_GLOBAL_INTERACTIVE\n- Once for every\nshell created"]
    BashLogout[shape = "note" label = ".bash_logout\n- Session end commands, once"]
    BashGlobalInteractiveProp [shape = "rarrow" label = "BASH_GLOBAL_INTERACTIVE"]
  }
  subgraph cluster_3 {
    label = "Non-Interactive Shell";
    BashEnv [shape = "rarrow" label = "BASH_ENV"]
  }

  BashCore -> BashInteractiveCore [arrowhead=none];
  BashProfile -> BashRc [arrowhead=none]
  BashRc -> BashLogout [arrowhead=none];
  BashEnv -> BashGlobalNonInteractive;
  BashGlobalInteractiveProp -> BashGlobalInteractive;
}
#+END_SRC
