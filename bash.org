...
digraph version5 {
  ...
  subgraph cluster_A {
    ...
    BashCore[label = "{BashCore|Standard language features\lWarn if unexpected version\l|For human & computer execution\l}"];
    BashNonInteractiveCore[label = "{BashNonInteractiveCore|Warn if interactive\nUnset interactive functions|Only for computer execution\l}"];
    BashInteractiveCore[label = "{BashInteractiveCore|Warn if non-interactive\nInterface\lDebugging\lHistory\lReadline\lPrompts\l|Only for human execution\l}"];
  }
  subgraph cluster_B {
...
Write this system using Bash 4.3.* features.

#+NAME: 86B3EC22-B47D-4E40-AFDD-B68AC6F77110
#+BEGIN_SRC sh
echo "$BASH_VERSION" | grep -q "^4\.3\." || { printf "\e[31mInadequate version of Bash found. Expected '4.3.*'; found '$BASH_VERSION'.\e[0m\n" 1>&2 ; sleep 5; }
#+END_SRC
...
My original plan was to provide aliases no matter how limited they may seem they
are useful even in their basic form. That worked well until I broke it on my
box using my Bash configuration (I think). The solution was to use functions
instead and explicitly disable aliases here.
...
Enable debugging information used both in non-interactive runs and interactive
debugging.
...
#+BEGIN_SRC sh
HISTTIMEFORMAT='+%Y_%m_%d_%H_%M_%S'
#+END_SRC
...
Easily use Java libraries.

#+NAME: FF5DCDD4-B856-4EDB-A9A1-BBE8D6A601CE
#+BEGIN_SRC sh
